...
#PUT usuário
@router.put('/{usuario_id}', response_model=UsuarioSchemaBase, status_code=status.HTTP_202_ACCEPT)
...
async def delete_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if usuario_del:
            ...
            await session.commit()
            return Response(status_code=status.HTTP_204_NO_CONTENT) #só é retornado dado porque o fastapi tem esse bug, mas não precisaria
        else:
            raise HTTPException(detail='usuário não encontrado', status_code=status.HTTP_404_NOT_FOUND)
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_session)):
    ...
    return JSONResponse(content={"access_token": criar_token_acesso(sub=usuario.id), "token-type": "bearer"}, status_code=status.HTTP_200_OK)